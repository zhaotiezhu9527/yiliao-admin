    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['business:userReport:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['business:userReport:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['business:userReport:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['business:userReport:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="userReportList" @selection-change="handleSelectionChange">
...
<script>
import { listUserReport, getUserReport, delUserReport, addUserReport, updateUserReport } from "@/api/business/userReport";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUserReport(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getUserReport(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateUserReport(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addUserReport(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户日报表编号为"' + ids + '"的数据项？').then(function() {
        return delUserReport(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('business/userReport/export', {
        ...this.queryParams
      }, `userReport_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
